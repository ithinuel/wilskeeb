...
type WakerVec = ArrayDeque<[(TimerInstant, Waker); super::TASK_COUNT]>;
                ...
                critical_section::with(|cs| {
                    if let Some((alarm, waker_vec)) = TIMER_WAKER.borrow_ref_mut(cs).as_mut() {
                        ...
                        let idx = waker_vec
                            .iter()
                            .enumerate()
                            .find_map(|(idx, &(d, _))| (d >= timestamp).then_some(idx))
                            .unwrap_or_else(|| waker_vec.len());

                        waker_vec
                            .insert(idx, (timestamp, cx.waker().clone()))
...
fn TIMER_IRQ_0() {
    ...
    critical_section::with(|cs| {
        if let Some((alarm, waker_vec)) = TIMER_WAKER.borrow_ref_mut(cs).as_mut() {
            ...
            let (_, waker) = waker_vec.pop_front().unwrap_or_else(|| unreachable!());
            waker.wake();

            while let Some((timestamp, next_waker)) = waker_vec.pop_front() {
                if timestamp >= (now + CUTOFF_DELAY) {
                    next_stop = Some(timestamp);

                    waker_vec
                        .push_front((timestamp, next_waker))
